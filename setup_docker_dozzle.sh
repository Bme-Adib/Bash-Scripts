...
Options:
  -e PORT          Expose Dozzle port to host system (default: N/A, do not expose)
  ...
  -d DIR           Target deployment directory (default: ./dozzle)
...
while getopts "e:s:n:d:yh" opt; do
    case "$opt" in
        e) HOST_PORT="$OPTARG" ;;
        s) SUBDOMAIN="$OPTARG" ;;
...
if [ -z "$HOST_PORT" ]; then
    if [ "$AUTO_CONFIRM" = true ]; then
        ...
        if [[ "$EXPOSE_PORT" =~ ^[Yy]$ ]]; then
            while true; do
                read -rp "Enter host port to bind Dozzle to [8888]: " INPUT_PORT
                INPUT_PORT=${INPUT_PORT:-8888}
...
if [[ "$deploy" =~ ^[Yy]$ ]]; then
    ...
    run_with_spinner "Deploying Dozzle container..." $DOCKER_COMPOSE_CMD up -d
    popd >/dev/null
    
    if validate_health "dozzle" "$HOST_PORT"; then
        log_success "Dozzle Log Viewer is running!"
    else
        log_error "Dozzle container failed health checks."
        exit 1
    fi
else
...
echo ""
draw_box "Dozzle Deployment Summary" "${summary_lines[@]}"
